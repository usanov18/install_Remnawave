...
## Что запускать в каком случае

| Ситуация | Что запускать |
| --- | --- |
| Нужна первая установка панели на новый сервер | `deploy-remnawave.sh` |
| Нужно полностью снести именно эту установку и начать заново | `cleanup-remnawave.sh` |
| Нужно перевезти существующую панель со старого сервера на новый | `migrate-remnawave.sh backup` на старом сервере, потом `migrate-remnawave.sh restore` на новом |
| Нужно просто обновить уже работающую панель до свежей версии | `update-remnawave.sh` |
